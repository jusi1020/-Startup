...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!rawKey) return res.status(500).json({ error: "KRIC_KEY is not configured on the server." });
    ...
    const upstream = await fetch(url, { headers:{ "User-Agent":"CampusGuide/1.0 (+vercel)" }, signal: ac.signal }).catch(e=>{ throw new Error(`Network error: ${e.message||e}`); });
    clearTimeout(timer);
    ...
    if (ct.includes("xml")) {
      ...
      let body; try { body=await upstream.json(); } catch { const text=await upstream.text(); body={ raw:text }; }
      res.setHeader("Cache-Control", cache);
      return res.status(200).json(body);
    }
  } catch (err) {
    const msg=String(err?.message||err); const isAbort=/aborted|AbortError/i.test(msg);
